...
internal static class PathGuard
{
    ...
    /// Returns (resolvedPath, null) on success, or (null, errorMessage) if the path escapes the allowed root.
    /// </summary>
    public static (string? Path, string? Error) Resolve(string? baseDirectory)
    {
        ...
        {
            return (null, $"Access denied: '{resolved}' is outside the allowed root '{AllowedRoot.TrimEnd(Path.DirectorySeparatorChar)}'.");
        }
